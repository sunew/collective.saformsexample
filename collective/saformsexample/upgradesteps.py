...
log = getLogger('collective.saformsexample:upgrades')
...
def runDefaultProfile(tool):
    """ Run default profile """
    site = tool.aq_parent
    runProfiles(site, ('collective.saformsexample:default',))


def runInitialSetupProfile(tool):
    """  """
    site = tool.aq_parent
    runProfiles(site, ('collective.saformsexample:initialsetup',))
...
def importDefaultProfileRegistry(tool):
    ...
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:default', 'plone.app.registry',
...
def importDefaultTypes(tool):
    ...
    setup_tool = getattr(site, 'portal_setup')
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:default', 'typeinfo',
        run_dependencies=False)
...
def importTinySettings(tool):
    ...
    setup_tool = getattr(site, 'portal_setup')
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:initialsetup', 'tinymce_settings',
        run_dependencies=False)
...
def importPortalTransforms(tool):
    ...
    setup_tool = getattr(site, 'portal_setup')
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:initialsetup', 'policy.portal_transforms',
        run_dependencies=False)
...
def importPortalproperties(tool):
    ...
    setup_tool = getattr(site, 'portal_setup')
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:initialsetup', 'propertiestool',
        run_dependencies=False)
...
def importActions(tool):
    ...
    setup_tool = getattr(site, 'portal_setup')
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:initialsetup', 'actions',
        run_dependencies=False)
...
def migratePortletsStep(tool):
    ...
    setup_tool = getattr(site, 'portal_setup')
    setup_tool.runImportStepFromProfile(
        'profile-collective.saformsexample:migrate', 'portlets',
        run_dependencies=False)
